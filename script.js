...
function showPreviousInvoices() {
    const historySection = document.getElementById('invoiceHistory');
    const historyList = document.getElementById('historyList');
    
    // Get invoice history from localStorage
    const invoiceHistory = JSON.parse(localStorage.getItem('invoiceHistory') || '[]');
    
    if (invoiceHistory.length === 0) {
        historyList.innerHTML = '<div class="no-history">No previous invoices found.</div>';
    } else {
        ...
        invoiceHistory.forEach(invoice => {
            ...
            historyItem.innerHTML = `
                ...
                <div class="history-item-details">
                    ...
                    <div class="history-item-detail">
                        <strong>Total Due:</strong> ${formatCurrency(invoice.totalDue)}
                    </div>
                    <div class="history-item-detail">
                ...
                <div class="history-item-actions">
                    <button class="history-action-btn" onclick="viewInvoice(${invoice.id})">👁️ View</button>
                    <button class="history-action-btn duplicate" onclick="duplicateInvoice(${invoice.id})">📋 Duplicate</button>
                    <button class="history-action-btn delete" onclick="deleteInvoice(${invoice.id})">🗑️ Delete</button>
                </div>
            `;
...
// View a specific invoice from history
function viewInvoice(invoiceId) {
    ...
    // Populate the invoice preview with historical data
    document.getElementById('displayInvoiceNumber').textContent = invoice.invoiceNumber;
    document.getElementById('displayInvoiceDate').textContent = invoice.invoiceDate;
    document.getElementById('displayDueDate').textContent = invoice.dueDate;
    document.getElementById('displayClientName').textContent = invoice.clientName;
    document.getElementById('displayClientAddress').textContent = invoice.clientAddress;
    document.getElementById('displayClientPhone').textContent = invoice.clientPhone;
    document.getElementById('displaySubTotal').textContent = formatCurrency(invoice.subtotal);
    document.getElementById('displayAlreadyPaid').textContent = formatCurrency(invoice.alreadyPaid);
    document.getElementById('displayTotalDue').textContent = formatCurrency(invoice.totalDue);
    document.getElementById('displayBankName').textContent = invoice.bankName;
    document.getElementById('displayAccountName').textContent = invoice.accountName;
    document.getElementById('displayAccountNumber').textContent = invoice.accountNumber;
    ...
    // Hide form and history, show invoice
    document.querySelector('.form-section').style.display = 'none';
    document.getElementById('invoiceHistory').style.display = 'none';
    document.getElementById('invoicePreview').style.display = 'block';
...
// Duplicate an invoice from history
function duplicateInvoice(invoiceId) {
    ...
    document.getElementById('dueDate').value = nextWeek.toISOString().split('T')[0]; // Next week
    ...
    document.getElementById('alreadyPaid').value = '0'; // Reset to 0
    ...
    // Show success message
    const successMsg = document.createElement('div');
    successMsg.innerHTML = '✅ Invoice duplicated successfully!';
    successMsg.style.cssText = 'position: fixed; top: 20px; right: 20px; background: #28a745; color: white; padding: 15px 20px; border-radius: 5px; z-index: 1000; font-size: 14px;';
    document.body.appendChild(successMsg);
...
function deleteInvoice(invoiceId) {
    ...
    let invoiceHistory = JSON.parse(localStorage.getItem('invoiceHistory') || '[]');
    invoiceHistory = invoiceHistory.filter(inv => inv.id !== invoiceId);
    localStorage.setItem('invoiceHistory', JSON.stringify(invoiceHistory));
    ...
    // Show success message
    const successMsg = document.createElement('div');
    successMsg.innerHTML = '✅ Invoice deleted successfully!';
    successMsg.style.cssText = 'position: fixed; top: 20px; right: 20px; background: #dc3545; color: white; padding: 15px 20px; border-radius: 5px; z-index: 1000; font-size: 14px;';
    document.body.appendChild(successMsg);
